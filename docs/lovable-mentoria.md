# Texto para colar no Lovable

O código da mentoria já está no GitHub e o Lovable puxa sozinho. O que falta é
só rodar o SQL no Supabase. É isso que este texto pede.

Troque `SEU@EMAIL.COM` pelo email da conta com que você faz login no painel
antes de colar.

---

Preciso que você rode uma migration no Supabase. Não altere nenhum arquivo .tsx
nem .ts do projeto: o código do frontend já está pronto e commitado, e se você
mexer nele vai quebrar. É só criar a migration e executar o SQL abaixo.
...
O INSERT é aberto ao papel anon de propósito, porque o formulário fica numa
página pública e a pessoa preenche sem estar logada. A policy limita o tamanho
dos campos e trava o status em 'novo' para robô não gravar lixo. A leitura é
só de admin: se anon pudesse dar SELECT, qualquer pessoa baixaria a lista de
telefones dos interessados.

Depois de rodar, regenere os tipos do Supabase para incluir a tabela
mentoria_leads.
...
1. Abra `/mentoria`, preencha o formulário e envie. Deve aparecer a tela do
   diagnóstico gratuito com o botão do WhatsApp.
2. Abra `/admin-panel` e vá na aba **Mentoria**. O lead que você acabou de
   enviar tem que estar lá com todas as respostas.

Se a aba abrir vazia mesmo tendo lead, é o papel de admin que não foi
concedido. Confira se o email no último comando do SQL é o mesmo com que você
faz login.
