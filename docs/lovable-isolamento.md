# Texto para colar no Lovable, isolamento entre clientes

Rode isto o quanto antes. Enquanto não rodar, qualquer pessoa com a chave
anônima do site consegue ler e apagar a tabela `products`.

Depois de rodar, faça também as duas coisas listadas no fim deste arquivo,
que não dá para fazer por SQL.

---

Preciso que você rode uma migration no Supabase e faça o deploy de duas Edge Functions que já estão no repositório. Não altere nenhum arquivo .tsx nem .ts do frontend: ele já está pronto e commitado.

**1. Crie uma migration e rode este SQL:**
...
**2. Faça o deploy das Edge Functions `cakto-webhook` e `admin-manage-user`.**
As duas criavam contas com uma senha fixa que estava escrita no código, e o
repositório é público. Agora geram senha aleatória. O deploy é obrigatório:
sem ele, as funções antigas continuam rodando com a senha fixa.

**3. Se existir a variável `SITE_URL` nas configurações de funções**, aponte
para o domínio do site. O `cakto-webhook` usa ela para montar o link de
definição de senha que vai para quem compra. Sem ela, cai num padrão que pode
estar errado.

---

## Fora do Lovable, e igualmente importante

**Troque a senha de todo mundo que já tem conta.** Todas as contas criadas
até agora nasceram com a mesma senha, que estava no repositório público. Quem
nunca trocou continua com ela. No painel admin, o botão de resetar senha agora
gera uma aleatória e mostra ela uma vez para você repassar. Ou peça para cada
pessoa usar o "Esqueci minha senha", que passou a existir na tela de login.

**O arquivo `.env` está no histórico do Git desde o primeiro commit.** Tirar
de vez exige reescrever o histórico com `git filter-repo`, o que quebra a
sincronia com o Lovable. Enquanto não fizer isso, considere as chaves que
estavam nele como conhecidas e troque as que puder trocar no painel do
Supabase.
...
Depois de rodar o SQL, rode esta consulta. Ela lista policy que libera geral,
e tem que voltar vazia:
...
Vale rodar de novo toda vez que criar tabela nova.
