-- Formulário de mentoria. O lead preenche sem estar logado, então o INSERT é
-- aberto ao papel anon. A leitura é só de admin: se anon pudesse dar SELECT,
-- qualquer pessoa baixaria a lista de telefones dos interessados.
...
CREATE TABLE IF NOT EXISTS public.mentoria_leads (
  ...
  dor TEXT NOT NULL,
  -- Acompanhamento do contato. Só o admin mexe nestes dois.
...
-- Qualquer visitante envia o formulário, mas dentro de limites de tamanho e
-- sempre entrando como 'novo'. Sem isso um robô conseguiria gravar texto de
-- megabytes ou já criar o lead marcado como fechado.
...
-- A leitura depende do papel de admin em public.user_roles. Se o painel abrir
-- vazio, é porque o seu usuário ainda não tem esse papel. Para conceder, rode
-- uma vez trocando o email:
--
